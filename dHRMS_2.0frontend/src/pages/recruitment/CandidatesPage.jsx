...
export default function CandidatesPage() {
  ...
  // Blacklist / Unblacklist Handler
  const handleToggleBlacklist = async (candidate) => {
    if (!candidate) return
    if (candidate.isBlacklisted) {
      try {
        await recruitmentService.unblacklistCandidate(candidate.candidateId)
        message.success('Candidate unblacklisted successfully.')
        loadCandidates()
        if (detailsOpen && selectedCandidate?.candidateId === candidate.candidateId) {
          openDetails(candidate)
        }
      } catch {
        message.error('Failed to unblacklist candidate.')
      }
    } else {
      Modal.confirm({
        ...
        content: (
          <div>
            <p>Are you sure you want to blacklist <strong>{candidate.firstName} {candidate.lastName || ''}</strong>?</p>
            <Input.TextArea
              id="blacklist-reason-textarea"
              rows={3}
              placeholder="Enter mandatory reason for blacklisting..."
              style={{ marginTop: 8 }}
            />
          </div>
        ),
        onOk: async () => {
          const reason = document.getElementById('blacklist-reason-textarea')?.value?.trim()
          if (!reason) {
            message.error('Blacklist reason is mandatory.')
            return Promise.reject()
          }
          try {
            await recruitmentService.blacklistCandidate(candidate.candidateId, { reason })
            message.success('Candidate blacklisted successfully.')
            loadCandidates()
            if (detailsOpen && selectedCandidate?.candidateId === candidate.candidateId) {
              openDetails(candidate)
            }
          } catch {
            message.error('Failed to blacklist candidate.')
          }
        }
      })
    }
    ...
    },
    {
      ...
      render: (_, r) => (
        <Space size="small">
          ...
          <Button
            type={r.isBlacklisted ? "default" : "dashed"}
            size="small"
            danger={!r.isBlacklisted}
            onClick={() => handleToggleBlacklist(r)}
            style={{ borderRadius: 6, fontSize: 11 }}
          >
            {r.isBlacklisted ? 'Unblacklist' : 'Blacklist'}
          </Button>
          <Tooltip title="Edit">
      ...
      >
        {detailsLoading ? (
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20 }}>
              ...
              <div>
                ...
                <Space style={{ marginTop: 4 }}>
                  {selectedCandidate.isBlacklisted ? (
                    <Tag color="volcano" style={{ fontWeight: 'bold' }}>⛔ Blacklisted Candidate</Tag>
                  ) : (
            ...
            {selectedCandidate.isBlacklisted && (
              <Alert
                message="⛔ Blacklisted Candidate"
                description={`Reason: ${selectedCandidate.blacklistReason || 'Violation of company terms or candidate policy.'}`}
                type="error"
                showIcon
