...
import { Table, Card, Button, Modal, Form, Select, Tag, Space, Row, Col, Typography, message, Steps, Alert } from 'antd'
import { CheckOutlined, CloseOutlined, BankOutlined, MoneyCollectOutlined } from '@ant-design/icons'
import { travelExpenseService } from '../../services/travelExpenseService'

const { Text } = Typography

export const TravelApprovalsContent = ({ batchModalOpen, setBatchModalOpen }) => {
  ...
  const handleCreateBatch = async () => {
    try {
      ...
      if (setBatchModalOpen) setBatchModalOpen(false)
      setSelectedRowKeys([])
      fetchData()
    } catch (err) {
      console.error(err)
  ...
  return (
    <div>
      ...
      <Card
        ...
        bordered={false}
        style={{ borderRadius: 8, boxShadow: '0 1px 3px rgba(0,0,0,0.05)', marginBottom: 20 }}
      >
        <Table
          rowSelection={{
            selectedRowKeys,
      ...
      <Modal
        ...
        onCancel={() => setBatchModalOpen && setBatchModalOpen(false)}
      >
...
export default TravelApprovalsContent
